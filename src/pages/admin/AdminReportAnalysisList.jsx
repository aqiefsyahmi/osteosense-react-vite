...
const AdminReportAnalysisList = () => {
  ...
  const sortedItems = [...doctorsWithPredictions]
    .filter(
      (item) =>
        item.fullname.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.phoneno.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.predict_count.toString().includes(searchQuery.toLowerCase())
    )
    .sort((a, b) => {
  ...
  const getDoctors = (doctorIdsWithPredictions) => {
    axios
      .get("http://127.0.0.1:5000/listdoctorswithcount")
          ...
          <div className="flex justify-end items-center gap-3">
            <div>
              ...
              <ItemsPerPageDropdown
                value={itemsPerPage}
                options={["6", "15", "20", "25", doctorsWithPredictions.length]}
          ...
          <table className="table table-striped table-border table-hover">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort("predict_count")}>
                  <div className="flex items-center">
                    Total Analysis Data
                    {sortConfig.key === "predict_count" ? (
                      sortConfig.direction === "ascending" ? (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          height="20px"
            ...
            <tbody>
              {sortedItems.length === 0 ? (
                ...
                sortedItems.map((doctor, key) => (
                  <tr key={key}>
                    <td>{indexOfFirstItem + key + 1}.</td>
                    <td>{doctor.fullname}</td>
                    <td>{doctor.email}</td>
                    <td>{doctor.phoneno}</td>
                    <td>{doctor.predict_count}</td>
                    <td>
                      <Link
